...
export function ContactAgendaAutocomplete(props: {
  schoolSlug: string;
  inputName: string;
  placeholder?: string;
  ...
  const displayValue = useMemo(() => {
    if (picked) return `${picked.fullName} · ${picked.phoneE164}`;
    return query;
  }, [picked, query]);
  ...
  return (
    <div className="relative">
      <div className="flex items-center gap-2">
        <input type="hidden" name={props.inputName} value={picked?.id ?? ""} />
        ...
        <input
          ...
          placeholder={props.placeholder ?? "Buscar por nombre o teléfono"}
          autoComplete="off"
        />
          ...
          <div className="p-2">
            ...
            {!loading && !error && results.length === 0 ? (
              <p className="px-2 py-2 text-xs text-muted">
                Sin resultados. Escribe el nombre o el teléfono para añadirlo manualmente.
              </p>
            ) : null}
      ...
      <p className="mt-1 text-xs text-muted">
        Busca en la agenda. Si no existe, completa nombre y teléfono para crearlo.
      </p>
    </div>
  );
}
